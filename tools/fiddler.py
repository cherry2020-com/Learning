...
class RawToPython(object):
    ...
    def __to_method_and_url(self):
        ...
        line_split = list(filter(lambda ch: ch,
        ...
        self.url_parse = urllib.parse.urlparse(self.url)
    ...
    def __to_body(self):
        if self.__lines[-2].strip():
            return
        body_data = urllib.parse.unquote(self.__lines[-1])
    ...
    def __set_url_param(self, param):
        ...
        if not self.try_real_simulation:
            url_param = dict([(k, v[0]) for k, v in urllib.parse.parse_qs(
    ...
    def __reset_req_param(self, req_param):
        if self.url != req_param['url']:
            if 'HOST' in req_param['headers']:
                req_param['headers']['HOST'] = urllib.parse.urlsplit(req_param['url']).netloc
            else:
                req_param['headers']['host'] = urllib.parse.urlsplit(req_param['url']).netloc
